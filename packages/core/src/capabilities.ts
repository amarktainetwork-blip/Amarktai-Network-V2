...
>> = {
  ...
  depth_estimation: { label: 'Depth Estimation', description: 'Estimate explicitly relative or calibrated metric depth from an authorised image artifact.', requiredFlags: [], inputContract: ['input.sourceImageArtifactId', 'input.outputMode', 'input.idempotencyKey'], outputType: 'image', artifactRequired: true, policyRequirement: 'standard' },
  keypoint_detection: { label: 'Keypoint Detection', description: 'Detect non-fabricated keypoints in an authorised image artifact.', requiredFlags: [], inputContract: ['input.sourceImageArtifactId', 'input.domain', 'input.idempotencyKey'], outputType: 'json', artifactRequired: true, policyRequirement: 'standard' },
  ...
  zero_shot_object_detection: { label: 'Zero-Shot Object Detection', description: 'Detect application-intent labels in an authorised image artifact.', requiredFlags: [], inputContract: ['input.sourceImageArtifactId', 'input.candidateLabels[]', 'input.idempotencyKey'], outputType: 'json', artifactRequired: true, policyRequirement: 'standard' },
  mask_generation: { label: 'Mask Generation', description: 'Generate validated masks from provider-neutral guidance over an authorised image artifact.', requiredFlags: [], inputContract: ['input.sourceImageArtifactId', 'input.guidance', 'input.idempotencyKey'], outputType: 'image', artifactRequired: true, policyRequirement: 'standard' },
  visual_document_retrieval: { label: 'Visual Document Retrieval', description: 'Return ranked, cited page or region evidence from an authorised document.', requiredFlags: [], inputContract: ['input.sourceDocumentArtifactId|input.ingestedDocumentId', 'input.query', 'input.idempotencyKey'], outputType: 'json', artifactRequired: true, policyRequirement: 'standard' },
  // Video, Avatar and 3D
  ...
  video_classification: { label: 'Video Classification', description: 'Classify sampled evidence from an authorised video artifact without inventing frame-accurate detections.', requiredFlags: [], inputContract: ['input.sourceVideoArtifactId', 'input.candidateLabels[]|input.governedTaxonomy', 'input.idempotencyKey'], outputType: 'json', artifactRequired: true, policyRequirement: 'standard' },
  ...
  research: { label: 'Research', description: 'Catalogue-only research contract; no executor is registered in the current release.', requiredFlags: [], inputContract: ['prompt', 'input.sources?'], outputType: 'document', artifactRequired: true, policyRequirement: 'standard' },
