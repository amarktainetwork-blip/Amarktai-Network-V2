...
export async function deepinfraEditImage(request: DeepInfraImageEditRequest): Promise<DeepInfraImageEditResponse> {
  ...
  try {
    ...
    form.append('image', new Blob([ownedBytes(request.imageBuffer)], { type: request.imageMimeType }), `source.${extensionForMime(request.imageMimeType)}`)
    ...
    if (request.maskBuffer?.length) {
      form.append('mask', new Blob([ownedBytes(request.maskBuffer)], { type: request.maskMimeType || 'image/png' }), `mask.${extensionForMime(request.maskMimeType || 'image/png')}`)
    }
