...
export async function discoverGroqProviderModels(options: DiscoveryAdapterOptions = {}): Promise<ProviderDiscoveryResult> {
  ...
  const staticModels = [
    ...
    modelFromProviderRecord({ provider: 'groq', modelId: 'whisper-large-v3', displayName: 'Whisper Large V3', rawProviderType: 'stt', category: 'audio', endpointSource: 'Groq official docs /openai/v1/audio/transcriptions', lastDiscoveredAt: timestamp, source: 'docs_fallback', providerClientExists: true, workerExecutorExists: true, endpointShapeKnown: true, requestShapeKnown: true, responseShapeKnown: true, artifactPersistenceExists: true, transportProfile: 'openai_audio_transcription_multipart' }),
    modelFromProviderRecord({ provider: 'groq', modelId: 'canopylabs/orpheus-v1-english', displayName: 'Orpheus V1 English', rawProviderType: 'tts', category: 'audio', endpointSource: 'Groq official docs /openai/v1/audio/speech', lastDiscoveredAt: timestamp, source: 'docs_fallback', providerClientExists: true, workerExecutorExists: true, endpointShapeKnown: true, requestShapeKnown: true, responseShapeKnown: true, artifactPersistenceExists: true, transportProfile: 'openai_audio_speech_binary' }),
  ]
