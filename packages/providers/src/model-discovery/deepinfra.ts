...
const DEEPINFRA_MODELS_ENDPOINT = 'https://api.deepinfra.com/v1/models'
...
function toModel(record: Record<string, unknown>, timestamp: string): ProviderDiscoveredModel {
  const modelId = stringField(record, ['id', 'model_name', 'model', 'name'])
...
export async function discoverDeepInfraProviderModels(options: DiscoveryAdapterOptions = {}): Promise<ProviderDiscoveryResult> {
  ...
  if (!options.live || !options.apiKey) {
    return skippedResult('deepinfra', DEEPINFRA_MODELS_ENDPOINT, [], ['DeepInfra documentation fallback is display-only. Authenticated GET /v1/models is required for executable catalogue truth.'])
  }
  try {
    const records = await fetchModelList(DEEPINFRA_MODELS_ENDPOINT, options.apiKey)
    const models = records.filter((record): record is Record<string, unknown> => typeof record === 'object' && record !== null)
      .map((record) => toModel(record, timestamp)).filter((model) => model.modelId)
    if (!models.length) return failedLiveResult('deepinfra', DEEPINFRA_MODELS_ENDPOINT, 'authenticated model list returned zero usable models', [])
    return {
      ...
      modelsEndpointScope: 'authenticated_account_catalogue', mode: 'live_model_list', source: 'live_endpoint',
      ...
      providerUniverseKnown: true, providerUniversePartiallyKnown: false, publicDocsUniverseKnown: false,
      authenticatedUniverseKnown: true, endpointSource: DEEPINFRA_MODELS_ENDPOINT, error: null,
      ...
      notes: ['Authenticated DeepInfra model inventory mapped by provider task type. Unknown task contracts remain visible but non-executable.'],
    }
  } catch (error) {
