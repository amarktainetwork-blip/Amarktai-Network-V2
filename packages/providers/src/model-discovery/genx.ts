...
function classifyGenx(record: Record<string, unknown>, modelId: string, rawType: string): {
  ...
  if (/image-to-video/.test(text)) return { taskType: 'image-to-video', capabilities: ['image_to_video'], inputs: ['text', 'image'], outputs: ['video'], transport: 'async_job_poll', endpointFamily: 'genx_generation_v1' }
  if (/video-to-video|reference-video/.test(text)) return { taskType: 'video-to-video', capabilities: ['video_to_video'], inputs: ['text', 'video'], outputs: ['video'], transport: 'async_job_poll', endpointFamily: 'genx_generation_v1' }
...
export async function discoverGenXProviderModels(options: DiscoveryAdapterOptions = {}): Promise<ProviderDiscoveryResult> {
  ...
  const staticModels = [
    modelFromProviderRecord({ provider: 'genx', executionProvider: 'genx', upstreamProvider: 'bytedance', modelId: 'seedance-v1-fast', displayName: 'Seedance V1 Fast', rawProviderType: 'text-to-video', inferredCapabilities: ['video_generation'], category: 'video', modalitiesIn: ['text'], modalitiesOut: ['video'], endpointSource: 'repo_static_genx_client', endpointFamily: 'genx_generation_v1', lastDiscoveredAt: timestamp, source: 'static_verified', discoverySource: 'static_verified', providerClientExists: true, workerExecutorExists: true, requestShapeKnown: true, responseShapeKnown: true, artifactPersistenceExists: true, transportProfile: 'async_job_poll' }),
  ...
  try {
    ...
    const models = [...byId.values()].map((record) => {
      ...
      return modelFromProviderRecord({
        ...
        category: classification.taskType,
