...
describe('GenX live model discovery classification', () => {
  it('classifies specialist media before generic video and voice routes without inventing executors', async () => {
    const records = [
      { id: 'digital-avatar-v2', category: 'video', description: 'Digital human avatar generation from a governed profile' },
    ...
    expect(byId.get('digital-avatar-v2')?.inferredCapabilities).toEqual(['avatar_generation'])
    ...
    for (const modelId of ['digital-avatar-v2', 'portrait-lip-sync-v1', 'instant-voice-clone-v1', 'voice-remix-v1', 'cinematic-sfx-v1']) {
