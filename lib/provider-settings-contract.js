...
export const FINAL_PROVIDER_IDS = APPROVED_PROVIDER_DEFINITIONS.map((provider) => provider.key)
...
export function normalizeProviderStatus(provider = {}) {
  ...
  return {
    ...
    runtimeEnabled: safeProvider.runtimeEnabled === true,
    ...
    credentialUsagePolicy: safeProvider.credentialUsagePolicy ?? (
      APPROVED_PROVIDER_DEFINITIONS.find((provider) => provider.key === safeProvider.providerKey)?.codingOnly
        ? 'coding_tools_only'
...
export function normalizeProviderStatuses(providers = []) {
  ...
  return FINAL_PROVIDER_IDS
    .map((providerKey, index) => byId.get(providerKey) ?? normalizeProviderStatus({
      providerKey,
      displayName: APPROVED_PROVIDER_DEFINITIONS.find((provider) => provider.key === providerKey)?.displayName ?? providerKey,
      enabled: false,
      configured: false,
      source: 'missing',
...
export function getHealthStatusClasses(healthStatus) {
  ...
  if (healthStatus === 'disabled' || healthStatus === 'gated' || healthStatus === 'runtime_restricted' || healthStatus === 'requires_review' || healthStatus === 'policy_restricted' || healthStatus === 'rate_limited' || healthStatus === 'insufficient_credit') return 'border-amber-500/30 text-amber-300'
  if (healthStatus === 'failed' || healthStatus === 'unhealthy' || healthStatus === 'authentication_failed') return 'border-rose-500/30 text-rose-300'
