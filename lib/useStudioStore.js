...
export const useStudioStore = create((set, get) => ({
  ...
  // ── The Core Asset Lifecycle ─────────────────────────────────
  // Step A: Button shows loading spinner, isGenerating = true
  // Step B: 2.5s delay (simulating API latency)
  // Step C: Success toast (caller handles this)
  // Step D: Push mock asset into generatedAssets
  // Step E: Asset Sidebar instantly reacts
  // Step F: Asset is draggable to Timeline
  simulateGeneration: async (capability, meta) => {
    const key = capability.split('.')[0]

    // Step A: Set generating state
    set((s) => ({
      generating: { ...s.generating, [key]: true },
      nodeStates: { ...s.nodeStates, [key]: 'processing' },
    }))

    // Step B: Simulate API latency
    ...
    const state = get()
    const trackMap = { video: 'video', voice: 'voice', music: 'music', avatar: 'video', image: 'video', longvideo: 'video' }
    ...
    // Step D + E: Push asset + auto-add to timeline
    set((s) => ({
    ...
    return asset // Return so caller can trigger toast (Step C)
  },
  ...
  // Step F: Drag-and-drop from Asset Sidebar to Timeline
  ...
  // ── Workspace Creation (App + Agent + Brand Vault) ───────────
  createWorkspace: async (workspaceData) => {
    ...
    const app = {
      ...
      tokenBalance: 1000,
      apiKeys: [{ prefix: `amk_${slug.slice(0, 6)}` }],
      createdAt: Date.now(),
    }
  ...
  fetchProviders: async () => {
    ...
    set({
      providers: [
        { id: 'genx', name: 'GenX', status: 'active', capabilities: ['video.generate', 'image.generate', 'avatar.generate', 'music.generate'], modelCount: 12, lastSynced: Date.now() - 3600000 },
        { id: 'groq', name: 'Groq', status: 'active', capabilities: ['text.chat', 'voice.tts', 'voice.stt'], modelCount: 8, lastSynced: Date.now() - 1800000 },
        { id: 'together', name: 'Together AI', status: 'active', capabilities: ['image.generate', 'image.edit', 'rag.ingest'], modelCount: 45, lastSynced: Date.now() - 7200000 },
        { id: 'mimo', name: 'MiMo', status: 'active', capabilities: ['text.reasoning', 'text.code'], modelCount: 2, lastSynced: Date.now() - 1200000 },
        { id: 'deepinfra', name: 'DeepInfra', status: 'active', capabilities: ['text.chat', 'image.generate', 'voice.stt', 'rag.ingest'], modelCount: 24, lastSynced: Date.now() - 900000 },
      ],
    })
  },
  fetchApps: async () => {
  ...
  fetchModels: async () => {
    ...
    set({
      models: [
        ...
        { id: 'm8', provider: 'deepinfra', name: 'llama-3.3-70b-versatile', capability: 'text.chat', status: 'available' },
        { id: 'm9', provider: 'mimo', name: 'mimo-reasoning', capability: 'text.reasoning', status: 'available' },
        { id: 'm10', provider: 'deepinfra', name: 'FLUX.1-schnell', capability: 'image.generate', status: 'available' },
        { id: 'm11', provider: 'deepinfra', name: 'bge-large-en-v1.5', capability: 'rag.ingest', status: 'available' },
        { id: 'm12', provider: 'deepinfra', name: 'whisper-large-v3', capability: 'voice.stt', status: 'available' },
        { id: 'm13', provider: 'mimo', name: 'mimo-code', capability: 'text.code', status: 'available' },
      ],
    })
  },
  testProvider: async () => {
