...
export default function AppConnections() {
  ...
  const create = async () => {
    ...
    await fetch('/api/connections', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, environment: env, dailyBudget: Number(budget) }) })
    toast.success('App connection created'); setName(''); load()
  }
  const genKey = async (id) => {
    const r = await fetch(`/api/connections/${id}/keys`, { method: 'POST' }); await r.json()
    toast.success('API key generated'); load()
  }
  const del = async (id) => { await fetch(`/api/connections/${id}`, { method: 'DELETE' }); load() }
  ...
  const simulate = async () => {
    ...
    setSimResult(await r.json()); toast.success('Payload simulated')
  }
