...
export default function SettingsPage() {
  return (
    <PageTransition className="space-y-6">
      <PageHeader title="Settings" subtitle="Manage provider credentials and inspect the deployment-controlled runtime policy." />
      ...
      <Tabs defaultValue="providers" className="space-y-4">
        ...
        <TabsContent value="storage">
          <Card className="border-white/[0.07] bg-white/[0.02] p-5">
            ...
            <p className="text-xs text-muted-foreground">Artifact storage is deployment-controlled and is not editable from the browser. Its writable state is reported by System Monitoring.</p>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="workers">
          <Card className="border-white/[0.07] bg-white/[0.02] p-5">
            ...
            <p className="mb-4 text-xs text-muted-foreground">Worker concurrency and runtime dependencies are deployment-controlled. Availability and heartbeat truth are reported by System Monitoring.</p>
            <h4 className="text-xs font-semibold mb-2">Declared runtime dependencies</h4>
            <div className="grid gap-2 sm:grid-cols-2">
              {OPEN_SOURCE_TOOLS.map((tool) => (
                <div key={tool.id} className="flex items-center justify-between rounded-md border border-white/[0.06] bg-black/20 px-3 py-2 text-xs">
                  <span>{tool.name}</span>
                  <Badge variant="outline" className="border-white/10 text-[9px]">Deployment controlled</Badge>
                </div>
              ))}
            </div>
          </Card>
        ...
        <TabsContent value="webhooks">
          <Card className="border-white/[0.07] bg-white/[0.02] p-5">
            ...
            <p className="text-xs text-muted-foreground">Webhook administration is outside this release candidate. No inactive configuration form or success response is exposed.</p>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="security">
          <Card className="border-white/[0.07] bg-white/[0.02] p-5">
            ...
            <p className="text-xs text-muted-foreground">Session, CORS, secret, and retention policy are deployment-controlled. Browser-managed provider credentials remain server-side encrypted and write-only.</p>
          </Card>
        </TabsContent>
      </Tabs>
    </PageTransition>
