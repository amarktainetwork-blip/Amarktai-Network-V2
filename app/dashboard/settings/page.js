...
export default function Settings() {
  ...
  useEffect(() => { fetch('/api/settings').then((r) => r.json()).then((d) => setS((p) => ({ ...p, ...(d.settings || {}) }))) }, [])
  const set = (k, v) => setS((p) => ({ ...p, [k]: v }))
  const save = async () => { await fetch('/api/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ settings: s }) }); toast.success('Settings saved') }
