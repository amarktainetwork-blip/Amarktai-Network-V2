...
export default function SpecialistVisionPage() {
  ...
  return <PageTransition className="space-y-6">
    ...
    <Card className="space-y-4 border-white/[0.07] bg-white/[0.02] p-5">
      ...
      {!(selected?.releaseCandidate || selected?.readyForDashboardExecution) && <div className="flex gap-2 rounded border border-amber-500/20 bg-amber-500/5 p-3 text-sm text-amber-200"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span><span className="block">{selected?.blockedReasons?.join(', ') || selected?.blocker || 'No eligible production route is registered.'}</span><span className="mt-1 block text-amber-100/80">A production-compatible executor is not registered. Local fixture proof is not live-provider proof.</span></span></div>}
