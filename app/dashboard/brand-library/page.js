...
export default function BrandLibraryPage() {
  return (
    <PageTransition className="space-y-6">
      <PageHeader title="Brand Library" subtitle="Brand scraping, BrandPack uploads, and brand intelligence are outside this release candidate." />
      ...
      <Card className="border-white/[0.07] bg-white/[0.02] p-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          No BrandPack execution or upload route is exposed. This screen cannot fabricate stored brand data.
        </p>
        <Button disabled variant="outline" className="mt-6 border-white/10 text-xs">
          <Lock className="mr-1.5 h-3 w-3" /> BrandPack execution disabled
        </Button>
      </Card>
          ...
          <AccordionContent>
            <div className="space-y-3 text-xs text-muted-foreground">
              ...
              <div className="rounded-md border border-white/[0.06] bg-black/20 p-3">
                ...
                <div>No BrandPack artifacts are created by the current release candidate.</div>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
