...
export default function CapabilitiesPage() {
  ...
  return (
    <PageTransition className="space-y-6">
      <PageHeader title="Capability Library" subtitle="Release readiness is projected by canonical backend truth; the full catalogue is not presented as launch-ready." />
            ...
            <Card key={group.family} className="border-white/[0.07] bg-white/[0.02] p-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {group.items.map((item) => {
                  ...
                  return (
                    <div key={item.key} aria-disabled={!ready} className={`rounded-lg border p-3 ${ready ? 'border-emerald-500/20 bg-emerald-500/[0.04]' : 'border-white/[0.06] bg-black/20 opacity-75'}`}>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <Badge variant="outline" className={`text-[9px] ${runtimeProofStatusClasses(proof)}`}>{runtimeProofStatusLabel(proof)}</Badge>
                      ...
                      <p className="mt-2 text-[9px] text-muted-foreground">
                        {proof.liveProven
                          ? `Deployed execution proven${proof.provider ? ` through ${proof.provider}` : ''}.`
                          : ready
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="dev" className="rounded-xl border border-white/[0.06] px-4">
          <AccordionTrigger className="py-3 text-xs"><span className="flex items-center gap-1.5 text-muted-foreground"><Settings className="h-3 w-3" />Developer matrix</span></AccordionTrigger>
          <AccordionContent>
            <div className="rounded-md border border-white/[0.06] bg-black/20 p-3 text-xs text-muted-foreground">
              <div className="mb-2 font-semibold">Canonical capability catalogue</div>
              <p className="text-[10px]">Catalogue membership describes contracts. It does not independently establish executor, app-grant, infrastructure, local-proof, or deployed live-proof status.</p>
              <p className="mt-2 text-[10px]">Retry, cancellation, provider routing, and model selection remain backend-controlled. Blocked capabilities stay visible for planning and disabled for execution.</p>
            </div>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
