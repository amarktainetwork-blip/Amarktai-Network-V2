...
export async function DELETE(request, { params }) {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/admin/providers/${encodeURIComponent(providerKey)}/key`, {
      method: 'DELETE',
    ...
    return NextResponse.json(
      { error: true, message: 'Backend unavailable. Provider key could not be cleared.' },
      { status: 502 },
    )
  }
}
