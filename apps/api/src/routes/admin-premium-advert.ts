...
async function selectPremiumRoute(capability: CapabilityKey, role: PremiumMediaRole): Promise<Omit<PremiumAdvertRoute, 'estimatedCreditsPerUnit'>> {
  ...
  const ranked = rankPremiumGenxModels(models.map((model) => ({
    ...
    executable: model.currentAvailability === 'available' && parseCapabilities(model.capabilitiesJson).includes(capability),
...
async function buildPlan(input: unknown): Promise<PlannedPremiumAdvert> {
  ...
  const [parentGrant, videoGrant, narrationGrant, musicGrant] = await Promise.all([
    resolveInternalDashboardCapabilityGrantSnapshot(APP_SLUG, 'long_form_video'),
    resolveInternalDashboardCapabilityGrantSnapshot(APP_SLUG, 'video_generation'),
    resolveInternalDashboardCapabilityGrantSnapshot(APP_SLUG, 'tts'),
    resolveInternalDashboardCapabilityGrantSnapshot(APP_SLUG, 'music_generation'),
  ])
  if (!parentGrant || !videoGrant || !narrationGrant || !musicGrant) {
    throw new Error('Premium advert requires long-form, video, narration and music grants for dashboard-long-form')
  }
  for (const grant of [parentGrant.grant, videoGrant.grant, narrationGrant.grant, musicGrant.grant]) {
    if (!grant.enabled || !grant.artifactWrite) throw new Error(`Premium advert grant is disabled or cannot write artifacts: ${grant.capability}`)
  }

  const [videoBase, narrationBase, musicBase, credential, providerStatus] = await Promise.all([
  ...
  const videoRoute: PremiumAdvertRoute = {
    ...videoBase,
    estimatedCreditsPerUnit: estimateGenxCredits(videoPricing, { generations: 1, videoSeconds: sceneSeconds }),
  }
  const narrationRoute: PremiumAdvertRoute = {
    ...narrationBase,
    estimatedCreditsPerUnit: estimateGenxCredits(narrationPricing, { generations: 1, audioSeconds: request.targetDurationSeconds }),
  }
  const musicRoute: PremiumAdvertRoute = {
    ...musicBase,
    estimatedCreditsPerUnit: estimateGenxCredits(musicPricing, { generations: 1, audioSeconds: request.targetDurationSeconds }),
  }
  return {
    request,
    plan: buildPremiumAdvertPlan({ request, videoRoute, narrationRoute, musicRoute, availableCredits: balance.availableCredits }),
    grants: { parent: parentGrant.grant, video: videoGrant.grant, narration: narrationGrant.grant, music: musicGrant.grant },
  }
}
...
function executionView(parent: Awaited<ReturnType<typeof prisma.job.findFirst>> & object, children: Awaited<ReturnType<typeof prisma.job.findMany>>, metadata: Record<string, unknown>) {
  ...
  return {
    ...
    candidates: candidates.map((job) => {
      const jobMetadata = safeJson(job.metadataJson)
      return {
        jobId: job.id,
        candidateId: jobMetadata.candidateId,
        sceneNumber: job.sceneNumber,
        candidateIndex: jobMetadata.candidateIndex,
        status: job.status,
...
export async function adminPremiumAdvertRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/admin/premium-advert/generate', async (request, reply) => {
    ...
    const parent = await prisma.job.create({
      data: {
        ...
        metadataJson: JSON.stringify({
          ...
          plan: built.plan,
          spend: built.plan.spend,
          createdAt,
          executionProfile: 'internal_dashboard',
          appGrantSnapshot: built.grants.parent,
        }),
    ...
    try {
      ...
      const jobs = await prisma.$transaction(async (tx) => {
          ...
          const job = await tx.job.create({
            data: {
              ...
              inputJson: JSON.stringify({
                ...
                aspectRatio: built.plan.aspectRatio,
                style: built.request.style,
                negativePrompt: candidate.negativePrompt,
                premiumAdvertCandidateId: candidate.candidateId,
              }),
              metadataJson: JSON.stringify(exactRouteMetadata(candidate.route, built.grants.video, {
                premiumAdvert: true,
                premiumAdvertCandidate: true,
                executionId,
                parentJobId: parent.id,
                candidateId: candidate.candidateId,
                candidateIndex: candidate.candidateIndex,
                sceneNumber: candidate.sceneNumber,
                sceneDurationSeconds: candidate.durationSeconds,
                planVersion: built.plan.version,
                spend: built.plan.spend,
              })),
              ...
              executionId,
              sceneNumber: candidate.sceneNumber,
              workflowPhase: 'premium_candidate_created',
            },
          })
          candidates.push(job)
        }
        const narration = await tx.job.create({
          data: {
            appSlug: APP_SLUG,
            capability: 'tts',
            prompt: built.plan.narration.text,
            ...
            metadataJson: JSON.stringify(exactRouteMetadata(built.plan.narration.route, built.grants.narration, {
              premiumAdvert: true, premiumAdvertNarration: true, executionId, parentJobId: parent.id, spend: built.plan.spend,
            })),
        ...
        const music = await tx.job.create({
          data: {
            ...
            prompt: built.plan.music.prompt,
            inputJson: JSON.stringify({
              originalPrompt: built.request.musicBrief,
              providerPrompt: `${built.plan.music.prompt} Premium cinematic master for a ${built.plan.targetDurationSeconds}-second brand advert. No vocals or spoken words.`,
              durationSeconds: built.plan.targetDurationSeconds,
              instrumentalOnly: true,
              vocalsRequested: false,
              masteringProfile: 'cinematic',
              outputFormat: 'wav',
            }),
            metadataJson: JSON.stringify(exactRouteMetadata(built.plan.music.route, built.grants.music, {
              premiumAdvert: true, premiumAdvertMusic: true, executionId, parentJobId: parent.id, spend: built.plan.spend,
            })),
      ...
      await prisma.job.update({
        where: { id: parent.id },
        data: { metadataJson: JSON.stringify({ ...safeJson(parent.metadataJson), subtitleArtifactId }) },
      })

      const q = getQueue()
      const allJobs = [...jobs.candidates, jobs.narration, jobs.music]
      for (const job of allJobs) {
        ...
        await q.add('process', payload, { ...DEFAULT_JOB_OPTIONS, jobId: job.id })
      ...
      return reply.status(202).send({
        ...
        parentJobId: parent.id,
        subtitleArtifactId,
        plan: publicPlan(built.plan),
        jobs: allJobs.map((job) => ({ id: job.id, capability: job.capability, sceneNumber: job.sceneNumber, status: job.status })),
      })
    } catch (error) {
  ...
  app.post('/api/admin/premium-advert/executions/:id/finalize', async (request, reply) => {
    ...
    const candidateJobs = loaded.children.filter((job) => safeJson(job.metadataJson).premiumAdvertCandidate === true && job.status === 'completed' && job.artifactId)
    ...
    if (!narration?.artifactId || !music?.artifactId || !subtitleArtifactId) {
      return reply.status(409).send({ error: true, message: 'Narration, music and subtitles must be complete before final assembly' })
    }

    const artifacts = await prisma.artifact.findMany({ where: { id: { in: candidateJobs.map((job) => job.artifactId!) } } })
    const artifactById = new Map(artifacts.map((artifact) => [artifact.id, artifact]))
    const evidence: PremiumCandidateEvidence[] = candidateJobs.flatMap((job) => {
      const jobMetadata = safeJson(job.metadataJson)
      const artifact = artifactById.get(job.artifactId!)
      if (!artifact) return []
      const artifactMetadata = safeJson(artifact.metadata)
      const width = Number(artifactMetadata.width ?? 0)
      const height = Number(artifactMetadata.height ?? 0)
      const durationSeconds = Number(artifactMetadata.duration ?? artifactMetadata.durationSeconds ?? 0)
      return [{
        candidateId: String(jobMetadata.candidateId),
        sceneNumber: Number(job.sceneNumber),
        model: job.model || String(jobMetadata.orchestraSelectedModel || ''),
        width: width > 0 ? width : null,
        height: height > 0 ? height : null,
        durationSeconds: durationSeconds > 0 ? durationSeconds : null,
        fileSizeBytes: artifact.fileSizeBytes,
        outputValidated: artifact.mimeType.startsWith('video/') && artifact.fileSizeBytes > 0 && width > 0 && height > 0 && durationSeconds > 0,
      }]
    })
    ...
    const selected: SelectedPremiumAdvertCandidate[] = winners.map((winner) => {
      const job = candidateJobs.find((candidate) => safeJson(candidate.metadataJson).candidateId === winner.candidateId)
      ...
      return {
        ...
        sceneNumber: winner.sceneNumber,
        jobId: job.id,
        artifactId: job.artifactId,
        provider: job.provider || 'genx',
        model: job.model || '',
    ...
    try {
      const assembled = await assemblePremiumAdvert({
        executionId: loaded.parent.executionId || loaded.parent.id,
        parentJobId: loaded.parent.id,
        plan,
        winners: selected,
        narrationArtifactId: narration.artifactId,
        musicArtifactId: music.artifactId,
        subtitleArtifactId,
      })
      ...
      const parent = await prisma.job.update({
        ...
        data: {
          status: 'completed', progress: 100, workflowPhase: 'completed', artifactId: assembled.artifactId,
          output: JSON.stringify({ artifactId: assembled.artifactId, winners: selected, assembly: assembled }),
          metadataJson: JSON.stringify(metadata), completedAt: new Date(), error: null,
        },
      })
      const refreshedChildren = await prisma.job.findMany({ where: { parentJobId: parent.id }, orderBy: [{ sceneNumber: 'asc' }, { createdAt: 'asc' }] })
      return reply.send({ success: true, reused: false, artifactId: assembled.artifactId, assembly: assembled, execution: executionView(parent, refreshedChildren, metadata) })
    } catch (error) {
