...
import { X, Image as ImageIcon, Film, Music, FileText, Download, GripVertical } from 'lucide-react'
        ...
        <div className="flex gap-1 border-b border-white/[0.06] px-3 py-2">
          {TYPE_FILTERS.map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`rounded-md px-2.5 py-1 text-xs capitalize transition ${filter === f ? 'bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              {f}
            </button>
          ))}
        </div>
