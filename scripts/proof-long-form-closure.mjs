...
function runStaticProof() {
  ...
  check(dashboard.includes("adminFetch('/api/admin/long-form-video/plan'") && dashboard.includes("adminFetch('/api/admin/long-form-video/approve'") && dashboard.includes('pollLong(data.executionId)'), 'dashboard creates plan, approves immutable plan, then polls execution')
  check(route.includes('providerCallsStarted: false') && dashboard.includes('planId') && dashboard.includes('versionHash'), 'API reports providerCallsStarted false and dashboard preserves planId/versionHash')
  check(dashboard.includes('/api/admin/long-form-video/executions/${executionId}') && dashboard.includes('pollLong'), 'dashboard polls canonical execution after approval')
