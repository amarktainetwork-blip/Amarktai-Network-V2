...
async function runStaticDiagnostic() {
  ...
  try {
    const content = readSource('apps/api/src/lib/long-form-assembly.ts')
    if (content.includes('export async function checkFfmpegAvailable')) pass('checkFfmpegAvailable exists')
    else fail('checkFfmpegAvailable exists')
    if (content.includes('export async function assembleLongFormVideo')) pass('assembleLongFormVideo exists')
    else fail('assembleLongFormVideo exists')
    if (content.includes('export async function assembleMultimediaLongFormVideo')) pass('assembleMultimediaLongFormVideo exists')
    else fail('assembleMultimediaLongFormVideo exists')
    if (content.includes('export async function resolveSceneArtifacts')) pass('resolveSceneArtifacts exists')
    else fail('resolveSceneArtifacts exists')
    if (content.includes('export async function resolveComponentArtifacts')) pass('resolveComponentArtifacts exists')
    else fail('resolveComponentArtifacts exists')
