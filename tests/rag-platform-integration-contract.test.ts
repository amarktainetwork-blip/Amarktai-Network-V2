...
const routes = readFileSync(new URL('../apps/api/src/routes/app-rag.ts', import.meta.url), 'utf8')
...
describe('durable RAG integration contract', () => {
  ...
  it('preflights and freezes every later capability before queueing paid work', () => {
    expect(routes).toContain("grantSnapshot(auth.app!.slug, 'rag_ingest', allowed)")
    expect(routes).toContain("grantSnapshot(auth.app!.slug, 'embeddings', allowed)")
    expect(routes).toContain("'reranking' as CapabilityKey")
    expect(routes).toContain("'question_answering' as CapabilityKey")
    ...
    expect(routes).toContain('rerankingGrantSnapshot: grants.get(\'reranking\')!.grant')
    expect(routes).toContain('answerGrantSnapshot: grants.get(\'question_answering\')!.grant')
  ...
  it('persists source bytes before provider-routed embedding execution', () => {
    expect(routes).toContain("subType: 'rag_source'")
    expect(routes).toContain("ragRole: 'source_embedding'")
    expect(routes).toContain("capability: 'embeddings'")
    expect(routes.indexOf('await saveArtifact')).toBeLessThan(routes.indexOf("await getQueue().add('process', payload"))
