...
describe('governed voice and avatar profile API contract', () => {
  ...
  it('validates app ownership, completion, bytes, type and MIME for every dependency', () => {
    ...
    expect(validation).toContain("dependencyError('PROFILE_ARTIFACT_NOT_FOUND'")
    expect(validation).toContain("dependencyError('PROFILE_ARTIFACT_TYPE_MISMATCH'")
    expect(validation).toContain("dependencyError('PROFILE_ARTIFACT_MIME_MISMATCH'")
  })
  ...
  it('allows only enabled catalogue voices on approved runtime providers with catalogue rights', () => {
    ...
    expect(validation).toContain("dependencyError('VOICE_CATALOGUE_PROVIDER_RESTRICTED'")
    expect(validation).toContain("dependencyError('VOICE_CATALOGUE_RIGHTS_UNVERIFIED'")
  })
