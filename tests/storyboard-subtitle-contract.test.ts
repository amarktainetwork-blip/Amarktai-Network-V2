...
describe('standalone subtitle contracts', () => {
  ...
  it('binds the generic Job route, central worker and authoritative fixture', () => {
    ...
    expect(fixture).toContain('Binary artifact proof must use its own response')
    expect(fixture).toContain('Buffer.from(await response.arrayBuffer())')
    expect(fixture).not.toContain('Buffer.from(await result.response.arrayBuffer())')
